...
namespace CIS.Service.Client.Services
{
    ...
    public class CisServiceProvider : BaseCisServiceProvider, ICisServiceProvider
    {
        ...
        public async Task<List<ContainerModel<T>>> LoadObjectListFnAsync<T>(string funcName, params object[] args) where T : IBody
        {
        ...
        public async Task<List<ContainerModel<T>>> LoadObjectListCodeFnAsync<T>(string funcName, params object[] args) where T : IBody
        {
            var uri = new Uri(new Uri(Settings.WebAPIEndpointAddress), $"{_controllerName}/LoadObjectListCodeFn/{typeof(T).Name}");
